...
namespace MRB.LanguageLearning
{
  public class DatabaseController
  {
    ...
    public Verb_Regular GetRandomVerb(int conjugationId)
    {
      List<Verb_Regular> verbStudyPool = new List<Verb_Regular>();

      foreach (var verbSuccessRate in _verbSuccessRates.Where(vsr => conjugationId == -1 || vsr.Key.ConjugationFK == conjugationId))
      {
    ...
    {
      List<Conjugation> conjugations = new List<Conjugation>(_dataContext.Conjugations);
      conjugations.Insert(0, Conjugation.Empty);
